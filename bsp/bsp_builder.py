...
class BSPTreeBuilder:
    ...
    def split_space(self, node: BSPNode, input_segments: list[Segment]):
        ...
        for segment in input_segments[1:]:
            ...
            # if the dominator is zero the lines are parallel
            denominator_is_zero = abs(denominator) < EPS

            # segments are collinear if they are parallel and numerator is zero
            ...
            if not denominator_is_zero:
                # intersection is the point on line segment where the line divides it
